...
### 1-3：String str="aaa"与 String str=new String("aaa")一样吗？`new String(“aaa”);`创建了几个字符串对象?
...
### 1-6：在使用 HashMap 的时候，用 String 做 key 有什么好处？

HashMap 内部实现是通过 key 的 hashcode 来确定 value 的存储位置，因为字符串是不可变的，所以当创建字符串时，它的 hashcode 被缓存下来，不需要再次计算，所以相比于其他对象更快。

## 2：包装类型

### 2-1：包装类型是什么？基本类型和包装类型有什么区别？
...
### 2-6：非new生成的Integer变量和new Integer()生成变量的对比
...
### 4-5：List<? extends T>和List <? super T>之间有什么区别 ?
...
### 4-6：可以把List`<String>`传递给一个接受List`<Object>`参数的方法吗？
...
### 4-8：判断`ArrayList<String>`与`ArrayList<Integer>`是否相等？
...
### 5-5：为什么还要显示指定serialVersionUID的值?
...
### 5-7：Java 序列化中如果有些字段不想进行序列化，怎么办？
...
### 6-2：非受检查异常(运行时异常)和受检查异常(一般异常)区别是什么？
...
### 6-4：NoClassDefFoundError 和 ClassNotFoundException 区别？
