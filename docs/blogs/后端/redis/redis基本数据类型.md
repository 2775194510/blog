...
![在这里插入图片描述](./redisImg/2020082010365930.png)
...
![在这里插入图片描述](./redisImg/20200820103713734.png)
...
![在这里插入图片描述](./redisImg/20200820103739584.png)
...
![在这里插入图片描述](./redisImg/20200820103804572.png)
...
![1](./redisImg/20200820103829446.png)

![在这里插入图片描述](./redisImg/20200820103851613.png)
...
1. 解压安装包
   ![在这里插入图片描述](./redisImg/20200820103922318.png)
2. 开启redis-server.exe
3. 启动redis-cli.exe测试![在这里插入图片描述](./redisImg/20200820103950934.png)
...
2. 解压Redis的安装包！程序一般放在 `/opt` 目录下
   ...
   ![在这里插入图片描述](./redisImg/20200820104016426.png)
...
![在这里插入图片描述](./redisImg/20200820104048327.png)

1. redis默认安装路径 `/usr/local/bin`![在这里插入图片描述](./redisImg/20200820104140692.png)

2. 将redis的配置文件复制到 程序安装目录 `/usr/local/bin/kconfig`下

   ![在这里插入图片描述](./redisImg/20200820104157817.png)

3. redis默认不是后台启动的，需要修改配置文件！

   ![在这里插入图片描述](./redisImg/20200820104213706.png)

4. 通过制定的配置文件启动redis服务

   ![在这里插入图片描述](./redisImg/20200820104228556.png)

5. 使用redis-cli连接指定的端口号测试，Redis的默认端口6379

   ![在这里插入图片描述](./redisImg/20200820104243223.png)

6. 查看redis进程是否开启

   ![在这里插入图片描述](./redisImg/20200820104300532.png)

7. 关闭Redis服务 `shutdown`

   ![在这里插入图片描述](./redisImg/20200820104314297.png)
...
![在这里插入图片描述](./redisImg/20200513214125892.png)
...
![在这里插入图片描述](./redisImg/20200820104343472.png)

### 基础知识

> redis默认有16个数据库

![在这里插入图片描述](./redisImg/20200820104357466.png)
...
![在这里插入图片描述](./redisImg/20200820104440398.png)
...
![在这里插入图片描述](./redisImg/20200513214531573.png)

那么就一定还存在一个RedisProperties类

![在这里插入图片描述](./redisImg/20200513214554661.png)

之前我们说SpringBoot2.x后默认使用Lettuce来替换Jedis，现在我们就能来验证了。

先看Jedis:

![在这里插入图片描述](./redisImg/20200513214607475.png)

@ConditionalOnClass注解中有两个类是默认不存在的，所以Jedis是无法生效的

然后再看Lettuce：

![在这里插入图片描述](./redisImg/20200513214618179.png)

完美生效。

现在我们回到RedisAutoConfiguratio

![img](./redisImg/2020051321462777.png)
...
![在这里插入图片描述](./redisImg/20200513214638238.png)

这是一些基本的配置属性。

![在这里插入图片描述](./redisImg/20200513214649380.png)

还有一些连接池相关的配置。注意使用时一定使用Lettuce的连接池。

![在这里插入图片描述](./redisImg/20200513214700372.png)
...
3. 测试结果
   ...
   **此时我们回到Redis查看数据时候，惊奇发现全是乱码，可是程序中可以正常输出：**
   ...
   ![在这里插入图片描述](./redisImg/20200513214734520.png)

    这时候就关系到存储对象的序列化问题，在网络中传输的对象也是一样需要序列化，否者就全是乱码。

   我们转到看那个默认的RedisTemplate内部什么样子：

   ![在这里插入图片描述](./redisImg/20200513214746506.png)

   在最开始就能看到几个关于序列化的参数。

   默认的序列化器是采用JDK序列化器

   ![在这里插入图片描述](./redisImg/20200513214757247.png)

   而默认的RedisTemplate中的所有序列化器都是使用这个序列化器：

   ![在这里插入图片描述](./redisImg/20200513214809494.png)
   ...
   - 直接调用RedisSerializer的静态方法来返回序列化器，然后set
     ...
     ![在这里插入图片描述](./redisImg/20200513214818682.png)

   - 自己new 相应的实现类，然后set

     ![在这里插入图片描述](./redisImg/20200513214827233.png)
...
![img](./redisImg/2020051321485460.png)

> 可以使用 include 组合多个配置问题

![在这里插入图片描述](./redisImg/20200513214902552.png)

> 网络配置

![在这里插入图片描述](./redisImg/20200513214912813.png)

> 日志输出级别

![img](./redisImg/20200513214923678.png)

> 日志输出文件

![在这里插入图片描述](./redisImg/20200513214933713.png)
...
![在这里插入图片描述](./redisImg/20200513214944964.png)

> RDB文件相关

![在这里插入图片描述](./redisImg/20200513214955679.png)

![在这里插入图片描述](./redisImg/20200513215006207.png)

> 主从复制

![在这里插入图片描述](./redisImg/20200513215016371.png)

> Security模块中进行密码设置

![在这里插入图片描述](./redisImg/20200513215026143.png)
...
![在这里插入图片描述](./redisImg/20200513215037918.png)

![在这里插入图片描述](./redisImg/20200513215047999.png)
...
![在这里插入图片描述](./redisImg/20200513215126515.jpg)
...
![在这里插入图片描述](./redisImg/20200513215141519.jpg)
...
![在这里插入图片描述](./redisImg/20200513215150892.jpg)
...
>
> ![在这里插入图片描述](./redisImg/20200513215205970.png)

![在这里插入图片描述](./redisImg/20200513215220858.jpg)

#### bgsave

`bgsave` 是异步进行，进行持久化的时候，`redis` 还可以将继续响应客户端请求 ；

![在这里插入图片描述](./redisImg/2020051321523151.jpg)
...
![在这里插入图片描述](./redisImg/20200513215523258.png)

当有新消息通过 PUBLISH 命令发送给频道 channel1 时， 这个消息就会被发送给订阅它的三个客户端：

![在这里插入图片描述](./redisImg/2020051321553483.png)
...
![在这里插入图片描述](./redisImg/2020051321554964.png)
...
![在这里插入图片描述](./redisImg/20200513215610163.png)
...
![在这里插入图片描述](./redisImg/20200513215637483.png)

然后主机上也能看到从机的状态：

![在这里插入图片描述](./redisImg/20200513215645778.png)

我们这里是使用命令搭建，是暂时的，==真实开发中应该在从机的配置文件中进行配置，==这样的话是永久的。

![在这里插入图片描述](./redisImg/20200513215654634.png)
...
![在这里插入图片描述](./redisImg/20200513215752444.png)

此时哨兵监视着我们的主机6379，当我们断开主机后：

![在这里插入图片描述](./redisImg/20200513215806972.png)
...
![在这里插入图片描述](./redisImg/20200513215824722.jpg)

**缓存空对象**

一次请求若在缓存和数据库中都没找到，就在缓存中方一个空对象用于处理后续这个请求。

![在这里插入图片描述](./redisImg/20200513215836317.jpg)
...
![在这里插入图片描述](./redisImg/20200513215850428.jpeg)
