...
```java
package com.xiaoze.exer.MenuTreeTest;
...
在 `application.properties` 中配置属性是中文，但是取出来是乱码

```java
	@Value("${lifeOrderPay.RL.recAcctNo}")
	private String recAcctNo;

	@Value("${lifeOrderPay.RL.recAcctName}")
	private String recAcctName;  // 中文值（但是是乱码）
```

```java
	// 将ISO 8851-1编码的字节转换为UTF-8编码的字节
    byte[] utf8Bytes = recAcctName.getBytes("ISO-8859-1");
	String utf8String = new String(utf8Bytes, "UTF-8");
```

## 12：入参通过反射转换jsonObject
...
/**
	 ...
	 * @param t
	 */
    public static <T> JSONObject reflect(T t){
        // 获取所有属性
...
```java
	//使用反射机制组装入参参数
	JSONObject str = YNHeatAssembler.reflect(req);
```

### 13：jsonObject转换javabean
```java
	public FsYNHeatPayOutput toPayData(JSONObject jsonData) {
		
		FsYNHeatPayOutput output = new FsYNHeatPayOutput();
		if (null != jsonData) {
			PayRspDto dto = JSON.toJavaObject(jsonData, PayRspDto.class);
			output = this.toPayRsp(dto);
		}
