<template><div><h2 id="_1-什么是jvm内存结构" tabindex="-1"><a class="header-anchor" href="#_1-什么是jvm内存结构" aria-hidden="true">#</a> 1. 什么是JVM内存结构？</h2>
<p><img src="http://blog-img.coolsen.cn/img/image-20210220111553294.png" alt=""></p>
...
<h2 id="_2-什么是jvm内存模型" tabindex="-1"><a class="header-anchor" href="#_2-什么是jvm内存模型" aria-hidden="true">#</a> 2. 什么是JVM内存模型？</h2>
<p><strong>Java 内存模型</strong>（下文简称 <strong>JMM</strong>）就是在底层处理器内存模型的基础上，定义自己的多线程语义。它明确指定了一组排序规则，来保证线程间的可见性。</p>
...
<p><img src="http://blog-img.coolsen.cn/img/image-20210329222941923.png" alt=""></p>
...
<h2 id="_3-heap-和stack-有什么区别" tabindex="-1"><a class="header-anchor" href="#_3-heap-和stack-有什么区别" aria-hidden="true">#</a> 3. heap 和stack 有什么区别？</h2>
...
<h2 id="_4-什么情况下会发生栈内存溢出" tabindex="-1"><a class="header-anchor" href="#_4-什么情况下会发生栈内存溢出" aria-hidden="true">#</a> 4. 什么情况下会发生栈内存溢出？</h2>
<p>1、栈是线程私有的，栈的生命周期和线程一样，每个方法在执行的时候就会创建一个栈帧，它包含局部变量表、操作数栈、动态链接、方法出口等信息，局部变量表又包括基本数据类型和对象的引用；
2、当线程请求的栈深度超过了虚拟机允许的最大深度时，会抛出StackOverFlowError异常，方法递归调用肯可能会出现该问题；
3、调整参数-xss去调整jvm栈的大小</p>
<h2 id="_5-谈谈对-oom-的认识-如何排查-oom-的问题" tabindex="-1"><a class="header-anchor" href="#_5-谈谈对-oom-的认识-如何排查-oom-的问题" aria-hidden="true">#</a> 5. 谈谈对 OOM 的认识？如何排查 OOM 的问题？</h2>
...
<h2 id="_6-谈谈-jvm-中的常量池" tabindex="-1"><a class="header-anchor" href="#_6-谈谈-jvm-中的常量池" aria-hidden="true">#</a> 6. 谈谈 JVM 中的常量池？</h2>
<p>JVM常量池主要分为<strong>Class文件常量池、运行时常量池，全局字符串常量池，以及基本类型包装类对象常量池</strong>。</p>
<ul>
...
<h2 id="_7-如何判断一个对象是否存活" tabindex="-1"><a class="header-anchor" href="#_7-如何判断一个对象是否存活" aria-hidden="true">#</a> 7. 如何判断一个对象是否存活？</h2>
<p>判断一个对象是否存活，分为两种算法1：引用计数法；2：可达性分析算法；</p>
<p><strong>引用计数法</strong>：
给每一个对象设置一个引用计数器，当有一个地方引用该对象的时候，引用计数器就+1，引用失效时，引用计数器就-1；当引用计数器为0的时候，就说明这个对象没有被引用，也就是垃圾对象，等待回收；
缺点：无法解决循环引用的问题，当A引用B，B也引用A的时候，此时AB对象的引用都不为0，此时也就无法垃圾回收，所以一般主流虚拟机都不采用这个方法；</p>
<p><strong>可达性分析法</strong>
从一个被称为GC Roots的对象向下搜索，如果一个对象到GC Roots没有任何引用链相连接时，说明此对象不可用，在java中可以作为GC Roots的对象有以下几种：</p>
<ul>
<li>虚拟机栈中引用的对象</li>
<li>方法区类静态属性引用的变量</li>
<li>方法区常量池引用的对象</li>
...
<h2 id="_8-强引用、软引用、弱引用、虚引用是什么-有什么区别" tabindex="-1"><a class="header-anchor" href="#_8-强引用、软引用、弱引用、虚引用是什么-有什么区别" aria-hidden="true">#</a> 8. 强引用、软引用、弱引用、虚引用是什么，有什么区别？</h2>
<ul>
...
<h2 id="_9-被引用的对象就一定能存活吗" tabindex="-1"><a class="header-anchor" href="#_9-被引用的对象就一定能存活吗" aria-hidden="true">#</a> 9. 被引用的对象就一定能存活吗？</h2>
<blockquote>
<p>不一定，看 Reference 类型，弱引用在 GC 时会被回收，软引用在内存不足的时候，即 OOM 前会被回收，但如果没有在 Reference Chain 中的对象就一定会被回收。</p>
</blockquote>
<h2 id="_10-java中的垃圾回收算法有哪些" tabindex="-1"><a class="header-anchor" href="#_10-java中的垃圾回收算法有哪些" aria-hidden="true">#</a> 10. Java中的垃圾回收算法有哪些？</h2>
<p>java中有四种垃圾回收算法，分别是标记清除法、标记整理法、复制算法、分代收集算法；
<strong>标记清除法</strong>：
第一步：利用可达性去遍历内存，把存活对象和垃圾对象进行标记；
第二步：在遍历一遍，将所有标记的对象回收掉；
特点：效率不行，标记和清除的效率都不高；标记和清除后会产生大量的不连续的空间分片，可能会导致之后程序运行的时候需分配大对象而找不到连续分片而不得不触发一次GC；</p>
<p><img src="http://blog-img.coolsen.cn/img/image-20210220111918592.png" alt=""></p>
<p><strong>标记整理法</strong>：
第一步：利用可达性去遍历内存，把存活对象和垃圾对象进行标记；
第二步：将所有的存活的对象向一段移动，将端边界以外的对象都回收掉；
特点：适用于存活对象多，垃圾少的情况；需要整理的过程，无空间碎片产生；</p>
<p><img src="http://blog-img.coolsen.cn/img/image-20210220111933505.png" alt=""></p>
<p><strong>复制算法</strong>：
将内存按照容量大小分为大小相等的两块，每次只使用一块，当一块使用完了，就将还存活的对象移到另一块上，然后在把使用过的内存空间移除；
特点：不会产生空间碎片；内存使用率极低；</p>
<p><strong>分代收集算法</strong>：
根据内存对象的存活周期不同，将内存划分成几块，java虚拟机一般将内存分成新生代和老生代，在新生代中，有大量对象死去和少量对象存活，所以采用复制算法，只需要付出少量存活对象的复制成本就可以完成收集；老年代中因为对象的存活率极高，没有额外的空间对他进行分配担保，所以采用标记清理或者标记整理算法进行回收；</p>
<p><strong>对比</strong></p>
<p><img src="http://blog-img.coolsen.cn/img/image-20210329224002527.png" alt="image-20210329224002527"></p>
<h2 id="_11-有哪几种垃圾回收器-各自的优缺点是什么" tabindex="-1"><a class="header-anchor" href="#_11-有哪几种垃圾回收器-各自的优缺点是什么" aria-hidden="true">#</a> 11. 有哪几种垃圾回收器，各自的优缺点是什么？</h2>
<p>垃圾回收器主要分为以下几种：Serial、ParNew、Parallel Scavenge、Serial Old、Parallel Old、CMS、G1；</p>
<ul>
<li>
<p>Serial:单线程的收集器，收集垃圾时，必须stop the world，使用复制算法。它的最大特点是在进行垃圾回收时，需要对所有正在执行的线程暂停（stop the world），对于有些应用是难以接受的，但是如果应用的实时性要求不是那么高，只要停顿的时间控制在N毫秒之内，大多数应用还是可以接受的，是client级别的默认GC方式。</p>
</li>
<li>
<p>ParNew:Serial收集器的多线程版本，也需要stop the world，复制算</p>
</li>
<li>
<p>Parallel Scavenge:新生代收集器，复制算法的收集器，并发的多线程收集器，目标是达到一个可控的吞吐量，和ParNew的最大区别是GC自动调节策略；虚拟机会根据系统的运行状态收集性能监控信息，动态设置这些参数，以提供最优停顿时间和最高的吞吐量；</p>
</li>
<li>
<p>Serial Old:Serial收集器的老年代版本，单线程收集器，使用标记整理算法。</p>
</li>
<li>
<p>Parallel Old：是Parallel Scavenge收集器的老年代版本，使用多线程，标记-整理算法。</p>
</li>
<li>
<p>CMS:是一种以获得最短回收停顿时间为目标的收集器，标记清除算法，运作过程：初始标记，并发标记，重新标记，并发清除，收集结束会产生大量空间碎片；</p>
</li>
<li>
<p>G1:标记整理算法实现，运作流程主要包括以下：初始标记，并发标记，最终标记，筛选回收。不会产生空间碎片，可以精确地控制停顿；G1将整个堆分为大小相等的多个Region（区域），G1跟踪每个区域的垃圾大小，在后台维护一个优先级列表，每次根据允许的收集时间，优先回收价值最大的区域，已达到在有限时间内获取尽可能高的回收效率；</p>
</li>
</ul>
<p><strong>垃圾回收器间的配合使用图：</strong></p>
<p><img src="http://blog-img.coolsen.cn/img/image-20210329224424220.png" alt=""></p>
<p><strong>各个垃圾回收器对比</strong>：</p>
<p><img src="http://blog-img.coolsen.cn/img/image-20210329230618579.png" alt=""></p>
<h2 id="_12-详细说一下cms的回收过程-cms的问题是什么" tabindex="-1"><a class="header-anchor" href="#_12-详细说一下cms的回收过程-cms的问题是什么" aria-hidden="true">#</a> 12. 详细说一下CMS的回收过程？CMS的问题是什么？</h2>
...
<h2 id="_13-详细说一下g1的回收过程" tabindex="-1"><a class="header-anchor" href="#_13-详细说一下g1的回收过程" aria-hidden="true">#</a> 13. 详细说一下G1的回收过程？</h2>
...
<h2 id="_14-jvm中一次完整的gc是什么样子的" tabindex="-1"><a class="header-anchor" href="#_14-jvm中一次完整的gc是什么样子的" aria-hidden="true">#</a> 14.  JVM中一次完整的GC是什么样子的？</h2>
<p>先描述一下Java堆内存划分。</p>
<p>在 Java 中，堆被划分成两个不同的区域：新生代 ( Young )、老年代 ( Old )，新生代默认占总空间的 1/3，老年代默认占 2/3。
新生代有 3 个分区：Eden、To Survivor、From Survivor，它们的默认占比是 8:1:1。</p>
<p>新生代的垃圾回收（又称Minor GC）后只有少量对象存活，所以选用复制算法，只需要少量的复制成本就可以完成回收。</p>
<p>老年代的垃圾回收（又称Major GC）通常使用“标记-清理”或“标记-整理”算法。</p>
<p><img src="http://blog-img.coolsen.cn/img/image-20210329225348086.png" alt=""></p>
<p>再描述它们之间转化流程：</p>
<ul>
<li>
<p>对象优先在Eden分配。当 eden 区没有足够空间进行分配时，虚拟机将发起一次 Minor GC。</p>
<ul>
...
<li>
<p>大对象直接进入老年代，大对象就是需要大量连续内存空间的对象（比如：字符串、数组），为了避免为大对象分配内存时由于分配担保机制带来的复制而降低效率。</p>
</li>
<li>
<p>老年代满了而<strong>无法容纳更多的对象</strong>，Minor GC 之后通常就会进行Full GC，Full GC 清理整个内存堆 – <strong>包括年轻代和老年代</strong>。</p>
</li>
</ul>
<h2 id="_15-minor-gc-和-full-gc-有什么不同呢" tabindex="-1"><a class="header-anchor" href="#_15-minor-gc-和-full-gc-有什么不同呢" aria-hidden="true">#</a> 15. Minor GC 和 Full GC 有什么不同呢？</h2>
<p>Minor GC：只收集新生代的GC。</p>
...
<h2 id="_16-介绍下空间分配担保原则" tabindex="-1"><a class="header-anchor" href="#_16-介绍下空间分配担保原则" aria-hidden="true">#</a> 16. 介绍下空间分配担保原则？</h2>
...
<p>通过下图来了解空间分配担保原则：</p>
<p><img src="http://blog-img.coolsen.cn/img/image-20210329230240201.png" alt=""></p>
<h2 id="_17-什么是类加载-类加载的过程" tabindex="-1"><a class="header-anchor" href="#_17-什么是类加载-类加载的过程" aria-hidden="true">#</a> 17. 什么是类加载？类加载的过程？</h2>
...
<li>
<p>验证：验证该class文件中的字节流信息复合虚拟机的要求，不会威胁到jvm的安全；</p>
</li>
<li>
<p>准备：为class对象的静态变量分配内存，初始化其初始值；</p>
</li>
<li>
<p>解析：该阶段主要完成符号引用转化成直接引用；</p>
</li>
<li>
<p>初始化：到了初始化阶段，才开始执行类中定义的java代码；初始化阶段是调用类构造器的过程；</p>
</li>
</ul>
<h2 id="_18-什么是类加载器-常见的类加载器有哪些" tabindex="-1"><a class="header-anchor" href="#_18-什么是类加载器-常见的类加载器有哪些" aria-hidden="true">#</a> 18. 什么是类加载器，常见的类加载器有哪些？</h2>
<p>类加载器是指：通过一个类的全限定性类名获取该类的二进制字节流叫做类加载器；类加载器分为以下四种：</p>
<ul>
<li>
<p>启动类加载器（BootStrapClassLoader）：用来加载java核心类库，无法被java程序直接引用；</p>
</li>
<li>
<p>扩展类加载器（Extension ClassLoader）：用来加载java的扩展库，java的虚拟机实现会提供一个扩展库目录，该类加载器在扩展库目录里面查找并加载java类；</p>
</li>
<li>
<p>系统类加载器（AppClassLoader）：它根据java的类路径来加载类，一般来说，java应用的类都是通过它来加载的；</p>
</li>
<li>
<p>自定义类加载器：由java语言实现，继承自ClassLoader；</p>
</li>
</ul>
<p><img src="http://blog-img.coolsen.cn/img/image-20210329231439914.png" alt=""></p>
<h2 id="_19-什么是双亲委派模型-为什么需要双亲委派模型" tabindex="-1"><a class="header-anchor" href="#_19-什么是双亲委派模型-为什么需要双亲委派模型" aria-hidden="true">#</a> 19. 什么是双亲委派模型？为什么需要双亲委派模型？</h2>
...
<h2 id="_20-列举一些你知道的打破双亲委派机制的例子-为什么要打破" tabindex="-1"><a class="header-anchor" href="#_20-列举一些你知道的打破双亲委派机制的例子-为什么要打破" aria-hidden="true">#</a> 20. 列举一些你知道的打破双亲委派机制的例子，为什么要打破？</h2>
<ul>
<li>
<p>JNDI 通过引入线程上下文类加载器，可以在 Thread.setContextClassLoader 方法设置，默认是应用程序类加载器，来加载 SPI 的代码。有了线程上下文类加载器，就可以完成父类加载器请求子类加载器完成类加载的行为。打破的原因，是为了 JNDI 服务的类加载器是启动器类加载，为了完成高级类加载器请求子类加载器（即上文中的线程上下文加载器）加载类。</p>
</li>
<li>
<p>Tomcat，应用的类加载器优先自行加载应用目录下的 class，并不是先委派给父加载器，加载不了才委派给父加载器。</p>
<p>tomcat之所以造了一堆自己的classloader，大致是出于下面三类目的：</p>
<ul>
<li>对于各个 <code v-pre>webapp</code>中的 <code v-pre>class</code>和 <code v-pre>lib</code>，需要相互隔离，不能出现一个应用中加载的类库会影响另一个应用的情况，而对于许多应用，需要有共享的lib以便不浪费资源。</li>
<li>与 <code v-pre>jvm</code>一样的安全性问题。使用单独的 <code v-pre>classloader</code>去装载 <code v-pre>tomcat</code>自身的类库，以免其他恶意或无意的破坏；</li>
<li>热部署。</li>
</ul>
<p>tomcat类加载器如下图：</p>
<p><img src="http://blog-img.coolsen.cn/img/image-20210329231930719.png" alt=""></p>
</li>
<li>
<p>OSGi，实现模块化热部署，为每个模块都自定义了类加载器，需要更换模块时，模块与类加载器一起更换。其类加载的过程中，有平级的类加载器加载行为。打破的原因是为了实现模块热替换。</p>
</li>
<li>
<p>JDK 9，Extension ClassLoader 被 Platform ClassLoader 取代，当平台及应用程序类加载器收到类加载请求，在委派给父加载器加载前，要先判断该类是否能够归属到某一个系统模块中，如果可以找到这样的归属关系，就要优先委派给负责那个模块的加载器完成加载。打破的原因，是为了添加模块化的特性。</p>
</li>
</ul>
<h2 id="_21-说一下-jvm-调优的命令" tabindex="-1"><a class="header-anchor" href="#_21-说一下-jvm-调优的命令" aria-hidden="true">#</a> 21.说一下 JVM 调优的命令？</h2>
<ul>
...
<li>jmap：jmap(JVM Memory Map)命令用于生成heap dump文件，如果不使用这个命令，还阔以使用-XX:+HeapDumpOnOutOfMemoryError参数来让虚拟机出现OOM的时候·自动生成dump文件。
jmap不仅能生成dump文件，还阔以查询finalize执行队列、Java堆和永久代的详细信息，如当前使用率、当前使用的是哪种收集器等。</li>
...
<h2 id="java对象创建过程" tabindex="-1"><a class="header-anchor" href="#java对象创建过程" aria-hidden="true">#</a> Java对象创建过程</h2>
<ol>
...
<p>https://jishuin.proginn.com/p/763bfbd35094</p>
<p>https://www.javanav.com/val/93550f179edb4a77bbf4d35faa6d560c.html</p>
<p>https://juejin.cn/post/6844903941805703181</p>
<p>https://www.cnblogs.com/chiangchou/p/jvm-2.html</p>
<p>https://juejin.cn/post/6844903887866953735</p>
<p>https://segmentfault.com/a/1190000023182342</p>
</div></template>
