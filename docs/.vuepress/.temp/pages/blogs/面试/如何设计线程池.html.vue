<template><div><h2 id="线程池核心设计与实现" tabindex="-1"><a class="header-anchor" href="#线程池核心设计与实现" aria-hidden="true">#</a> 线程池核心设计与实现</h2>
<h3 id="_2-1-总体设计" tabindex="-1"><a class="header-anchor" href="#_2-1-总体设计" aria-hidden="true">#</a> 2.1 总体设计</h3>
<p>Java中的线程池核心实现类是ThreadPoolExecutor</p>
...
<h3 id="_2-2-生命周期管理" tabindex="-1"><a class="header-anchor" href="#_2-2-生命周期管理" aria-hidden="true">#</a> 2.2 生命周期管理</h3>
<p>线程池运行的状态，并不是用户显式设置的，而是伴随着线程池的运行，由内部来维护。线程池内部使用一个变量维护两个值：运行状态(runState)和线程数量 (workerCount)。</p>
<p>如下代码所示：</p>
<div class="language-Java line-numbers-mode" data-ext="Java"><pre v-pre class="language-Java"><code>private final AtomicInteger ctl = new AtomicInteger(ctlOf(RUNNING, 0));
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><code v-pre>ctl</code>这个AtomicInteger类型，是对线程池的运行状态和线程池中有效线程的数量进行控制的一个字段， 它同时包含两部分的信息：线程池的运行状态 (runState) 和线程池内有效线程的数量 (workerCount)，高3位保存runState，低29位保存workerCount，两个变量之间互不干扰。用一个变量去存储两个值，可避免在做相关决策时，出现不一致的情况，不必为了维护两者的一致，而占用锁资源。</p>
...
<h3 id="_2-3-任务执行机制" tabindex="-1"><a class="header-anchor" href="#_2-3-任务执行机制" aria-hidden="true">#</a> 2.3 任务执行机制</h3>
...
<h3 id="_2-4-worker线程管理" tabindex="-1"><a class="header-anchor" href="#_2-4-worker线程管理" aria-hidden="true">#</a> 2.4 Worker线程管理</h3>
<p>2.4.1 Worker线程</p>
<p>线程池为了掌握线程的状态并维护线程的生命周期，设计了线程池内的工作线程Worker。我们来看一下它的部分代码：</p>
<div class="language-Java line-numbers-mode" data-ext="Java"><pre v-pre class="language-Java"><code>private final class Worker extends AbstractQueuedSynchronizer implements Runnable{
    final Thread thread;//Worker持有的线程
    Runnable firstTask;//初始化的任务，可以为null
}
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Worker这个工作线程，实现了Runnable接口，并持有一个线程thread，一个初始化的任务firstTask。thread是在调用构造方法时通过ThreadFactory来创建的线程，可以用来执行任务；firstTask用它来保存传入的第一个任务，这个任务可以有也可以为null。如果这个值是非空的，那么线程就会在启动初期立即执行这个任务，也就对应核心线程创建时的情况；如果这个值是null，那么就需要创建一个线程去执行任务列表（workQueue）中的任务，也就是非核心线程的创建。</p>
...
<div class="language-Java line-numbers-mode" data-ext="Java"><pre v-pre class="language-Java"><code>try {
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>线程回收的工作是在processWorkerExit方法完成的。</p>
...
<p>图11 执行任务流程</p>
<h2 id="线程池参数设置" tabindex="-1"><a class="header-anchor" href="#线程池参数设置" aria-hidden="true">#</a> 线程池参数设置</h2>
<h3 id="_1-常规设置" tabindex="-1"><a class="header-anchor" href="#_1-常规设置" aria-hidden="true">#</a> 1. 常规设置</h3>
<p>分 IO 密集型任务或者分 CPU 密集型任务</p>
<h4 id="cpu-密集型任务" tabindex="-1"><a class="header-anchor" href="#cpu-密集型任务" aria-hidden="true">#</a> <strong>CPU 密集型任务</strong></h4>
<p><code v-pre>CPU密集型</code>也叫<code v-pre>计算密集型</code>，指的是系统的硬盘、内存性能相对CPU要好很多，此时，系统运作大部分的状况是CPU Loading 100%，CPU要读/写I/O(硬盘/内存)，I/O在很短的时间就可以完成，而CPU还有许多运算要处理，CPU Loading 很高。</p>
<p><strong>CPU密集型：corePoolSize = CPU核数 + 1</strong></p>
<p>《Java并发编程实战》一书中给出的原因是：**即使当计算（CPU）密集型的线程偶尔由于页缺失故障或者其他原因而暂停时，这个“额外”的线程也能确保 CPU 的时钟周期不会被浪费。**把它理解为一个备份的线程就行了。</p>
<p>注意：这个地方还有个需要注意的小点就是，如果你的服务器上部署的不止一个应用，你就得考虑其他的应用的线程池配置情况。</p>
<p>经过精密的计算，你咔一下设置为核心数，结果项目部署上去了，发现还有其他的应用在和你抢 CPU。</p>
<h4 id="io-密集型任务" tabindex="-1"><a class="header-anchor" href="#io-密集型任务" aria-hidden="true">#</a> <strong>IO 密集型任务</strong></h4>
<p>IO密集型的话，是指系统大部分时间在跟I/O交互，而这个时间线程不会占用CPU来处理，即在这个时间范围内，可以由其他线程来使用CPU，因而可以多配置一些线程。</p>
<p>业界的一些线程池参数配置方案：</p>
...
<h3 id="_2-线程池参数动态化" tabindex="-1"><a class="header-anchor" href="#_2-线程池参数动态化" aria-hidden="true">#</a> 2. <strong>线程池参数动态化</strong></h3>
<p>可以将修改线程池参数的成本降下来，这样至少可以发生故障的时候可以快速调整从而缩短故障恢复的时间。可以将线程池的参数从代码中迁移到分布式配置中心上，实现线程池参数可动态配置和即时生效，线程池参数动态化前后的参数修改流程对比如下：</p>
...
<h4 id="_3-2-1-整体设计" tabindex="-1"><a class="header-anchor" href="#_3-2-1-整体设计" aria-hidden="true">#</a> <strong>3.2.1 整体设计</strong></h4>
...
<h4 id="_3-2-2-参数动态化" tabindex="-1"><a class="header-anchor" href="#_3-2-2-参数动态化" aria-hidden="true">#</a> 3.2.2 <strong>参数动态化</strong></h4>
...
<ol>
<li>
<p>问题一：线程池被创建后里面有线程吗？如果没有的话，你知道有什么方法对线程池进行预热吗？</p>
...
<p>https://blog.csdn.net/z55887/article/details/79060070</p>
<p>https://www.cnblogs.com/thisiswhy/p/12690630.html</p>
<p>http://ifeve.com/how-to-calculate-threadpool-size/</p>
</div></template>
