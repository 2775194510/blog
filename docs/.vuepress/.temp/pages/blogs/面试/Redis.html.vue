...
<h3 id="_1-redis是什么-简述它的优缺点" tabindex="-1"><a class="header-anchor" href="#_1-redis是什么-简述它的优缺点" aria-hidden="true">#</a> 1. Redis是什么？简述它的优缺点？</h3>
...
<h3 id="_2-redis为什么这么快" tabindex="-1"><a class="header-anchor" href="#_2-redis为什么这么快" aria-hidden="true">#</a> 2. Redis为什么这么快？</h3>
<ul>
<li>
<p>内存存储：Redis是使用内存(in-memeroy)存储，没有磁盘IO上的开销。数据存在内存中，类似于 HashMap，HashMap 的优势就是查找和操作的时间复杂度都是O(1)。</p>
</li>
<li>
<p>单线程实现（ Redis 6.0以前）：Redis使用单个线程处理请求，避免了多个线程之间线程切换和锁资源争用的开销。注意：单线程是指的是在核心网络模型中，网络请求模块使用一个线程来处理，即一个线程处理所有网络请求。</p>
</li>
<li>
<p>非阻塞IO：Redis使用多路复用IO技术，将epoll作为I/O多路复用技术的实现，再加上Redis自身的事件处理模型将epoll中的连接、读写、关闭都转换为事件，不在网络I/O上浪费过多的时间。</p>
</li>
<li>
<p>优化的数据结构：Redis有诸多可以直接应用的优化数据结构的实现，应用层可以直接使用原生的数据结构提升性能。</p>
</li>
<li>
<p>使用底层模型不同：Redis直接自己构建了 VM (虚拟内存)机制 ，因为一般的系统调用系统函数的话，会浪费一定的时间去移动和请求。</p>
<blockquote>
...
<h3 id="_3-redis相比memcached有哪些优势" tabindex="-1"><a class="header-anchor" href="#_3-redis相比memcached有哪些优势" aria-hidden="true">#</a> 3. Redis相比Memcached有哪些优势？</h3>
<ul>
<li>
<p>数据类型：Memcached所有的值均是简单的字符串，Redis支持更为丰富的数据类型，支持string(字符串)，list(列表)，Set(集合)、Sorted Set(有序集合)、Hash(哈希)等。</p>
</li>
<li>
<p>持久化：Redis支持数据落地持久化存储，可以将内存中的数据保持在磁盘中，重启的时候可以再次加载进行使用。 memcache不支持数据持久存储 。</p>
</li>
<li>
<p>集群模式：Redis提供主从同步机制，以及 Cluster集群部署能力，能够提供高可用服务。Memcached没有原生的集群模式，需要依靠客户端来实现往集群中分片写入数据</p>
</li>
<li>
<p>性能对比：Redis的速度比Memcached快很多。</p>
</li>
<li>
<p>网络IO模型：Redis使用单线程的多路 IO 复用模型，Memcached使用多线程的非阻塞IO模式。</p>
</li>
<li>
...
<h3 id="_4-为什么要用-redis-做缓存" tabindex="-1"><a class="header-anchor" href="#_4-为什么要用-redis-做缓存" aria-hidden="true">#</a> 4. 为什么要用 Redis 做缓存？</h3>
...
<h3 id="_5-为什么要用-redis-而不用-map-guava-做缓存" tabindex="-1"><a class="header-anchor" href="#_5-为什么要用-redis-而不用-map-guava-做缓存" aria-hidden="true">#</a> 5. 为什么要用 Redis 而不用 map/guava 做缓存?</h3>
...
<h3 id="_6-redis的常用场景有哪些" tabindex="-1"><a class="header-anchor" href="#_6-redis的常用场景有哪些" aria-hidden="true">#</a> 6. Redis的常用场景有哪些?</h3>
...
<h3 id="_7-redis的数据类型有哪些" tabindex="-1"><a class="header-anchor" href="#_7-redis的数据类型有哪些" aria-hidden="true">#</a> 7. Redis的数据类型有哪些？</h3>
<p>有五种常用数据类型：String、Hash、Set、List、SortedSet。以及三种特殊的数据类型：Bitmap、HyperLogLog、Geospatial ，其中HyperLogLog、Bitmap的底层都是 String 数据类型，Geospatial 的底层是 Sorted Set 数据类型。</p>
...
<h3 id="_8-redis持久化机制" tabindex="-1"><a class="header-anchor" href="#_8-redis持久化机制" aria-hidden="true">#</a> 8. Redis持久化机制？</h3>
<p>为了能够重用Redis数据，或者防止系统故障，我们需要将Redis中的数据写入到磁盘空间中，即持久化。</p>
<p>Redis提供了两种不同的持久化方法可以将数据存储在磁盘中，一种叫快照<code v-pre>RDB</code>，另一种叫只追加文件<code v-pre>AOF</code>。</p>
<p><strong>RDB</strong></p>
<p>在指定的时间间隔内将内存中的数据集快照写入磁盘(<code v-pre>Snapshot</code>)，它恢复时是将快照文件直接读到内存里。</p>
<p><strong>优势</strong>：适合大规模的数据恢复；对数据完整性和一致性要求不高</p>
<p><strong>劣势</strong>：在一定间隔时间做一次备份，所以如果Redis意外<code v-pre>down</code>掉的话，就会丢失最后一次快照后的所有修改。</p>
...
<ul>
<li>每修改同步：<code v-pre>appendfsync always</code> 同步持久化，每次发生数据变更会被立即记录到磁盘，性能较差但数据完整性比较好</li>
<li>每秒同步：<code v-pre>appendfsync everysec</code> 异步操作，每秒记录，如果一秒内宕机，有数据丢失</li>
<li>不同步：<code v-pre>appendfsync no</code>   从不同步</li>
</ul>
<p><strong>劣势</strong></p>
<ul>
<li>相同数据集的数据而言<code v-pre>aof</code>文件要远大于<code v-pre>rdb</code>文件，恢复速度慢于<code v-pre>rdb</code></li>
<li><code v-pre>aof</code>运行效率要慢于<code v-pre>rdb</code>，每秒同步策略效率较好，不同步效率和<code v-pre>rdb</code>相同</li>
</ul>
<h3 id="_9-如何选择合适的持久化方式" tabindex="-1"><a class="header-anchor" href="#_9-如何选择合适的持久化方式" aria-hidden="true">#</a> 9. 如何选择合适的持久化方式</h3>
<ul>
...
<h3 id="_10-redis持久化数据和缓存怎么做扩容" tabindex="-1"><a class="header-anchor" href="#_10-redis持久化数据和缓存怎么做扩容" aria-hidden="true">#</a> 10. Redis持久化数据和缓存怎么做扩容？</h3>
<ul>
<li>
...
<h2 id="过期键的删除策略、淘汰策略" tabindex="-1"><a class="header-anchor" href="#过期键的删除策略、淘汰策略" aria-hidden="true">#</a> 过期键的删除策略、淘汰策略</h2>
<h3 id="_11-redis过期键的删除策略" tabindex="-1"><a class="header-anchor" href="#_11-redis过期键的删除策略" aria-hidden="true">#</a> 11. Redis过期键的删除策略</h3>
...
<h3 id="_12-redis-key的过期时间和永久有效分别怎么设置" tabindex="-1"><a class="header-anchor" href="#_12-redis-key的过期时间和永久有效分别怎么设置" aria-hidden="true">#</a> 12. Redis key的过期时间和永久有效分别怎么设置？</h3>
...
<h3 id="_13-redis内存淘汰策略" tabindex="-1"><a class="header-anchor" href="#_13-redis内存淘汰策略" aria-hidden="true">#</a> 13. Redis内存淘汰策略</h3>
<p>Redis是不断的删除一些过期数据，但是很多没有设置过期时间的数据也会越来越多，那么Redis内存不够用的时候是怎么处理的呢？答案就是淘汰策略。此类的</p>
...
<h2 id="缓存异常" tabindex="-1"><a class="header-anchor" href="#缓存异常" aria-hidden="true">#</a> 缓存异常</h2>
<blockquote>
<p>缓存异常有四种类型，分别是缓存和数据库的数据不一致、缓存雪崩、缓存击穿和缓存穿透。</p>
</blockquote>
<h3 id="_14-如何保证缓存与数据库双写时的数据一致性" tabindex="-1"><a class="header-anchor" href="#_14-如何保证缓存与数据库双写时的数据一致性" aria-hidden="true">#</a> 14. 如何保证缓存与数据库双写时的数据一致性？</h3>
<blockquote>
...
<p>目前主要用第三和第四种方案。</p>
<h3 id="_15-先删除缓存-后更新数据库" tabindex="-1"><a class="header-anchor" href="#_15-先删除缓存-后更新数据库" aria-hidden="true">#</a> 15. 先删除缓存，后更新数据库</h3>
...
<h4 id="答案一-延时双删" tabindex="-1"><a class="header-anchor" href="#答案一-延时双删" aria-hidden="true">#</a> 答案一：延时双删</h4>
<p>最简单的解决办法延时双删</p>
<p>使用伪代码如下：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>public void write(String key,Object data){
		Redis.delKey(key);
	    db.updateData(data);
	    Thread.sleep(1000);
	    Redis.delKey(key);
	}
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>转化为中文描述就是
（1）先淘汰缓存
（2）再写数据库（这两步和原来一样）
（3）休眠1秒，再次淘汰缓存，这么做，可以将1秒内所造成的缓存脏数据，再次删除。确保读请求结束，写请求可以删除读请求造成的缓存脏数据。自行评估自己的项目的读数据业务逻辑的耗时，写数据的休眠时间则在读数据业务逻辑的耗时基础上，加几百ms即可。</p>
...
<h4 id="答案二-更新与读取操作进行异步串行化" tabindex="-1"><a class="header-anchor" href="#答案二-更新与读取操作进行异步串行化" aria-hidden="true">#</a> 答案二： <strong>更新与读取操作进行异步串行化</strong></h4>
...
<h3 id="_16-先更新数据库-后删除缓存" tabindex="-1"><a class="header-anchor" href="#_16-先更新数据库-后删除缓存" aria-hidden="true">#</a> 16. 先更新数据库，后删除缓存</h3>
...
<h3 id="_17-什么是缓存击穿" tabindex="-1"><a class="header-anchor" href="#_17-什么是缓存击穿" aria-hidden="true">#</a> 17. 什么是缓存击穿?</h3>
...
<ul>
<li>
<p>在缓存失效后，通过互斥锁或者队列来控制读数据写缓存的线程数量，比如某个key只允许一个线程查询数据和写缓存，其他线程等待。这种方式会阻塞其他的线程，此时系统的吞吐量会下降</p>
</li>
<li>
<p>热点数据缓存永远不过期。永不过期实际包含两层意思：</p>
<ul>
...
<h3 id="_18-什么是缓存穿透" tabindex="-1"><a class="header-anchor" href="#_18-什么是缓存穿透" aria-hidden="true">#</a> 18. 什么是缓存穿透?</h3>
...
<h3 id="_19-什么是缓存雪崩" tabindex="-1"><a class="header-anchor" href="#_19-什么是缓存雪崩" aria-hidden="true">#</a> 19. 什么是缓存雪崩?</h3>
...
<ul>
<li>
<p>均匀过期：设置不同的过期时间，让缓存失效的时间尽量均匀，避免相同的过期时间导致缓存雪崩，造成大量数据库的访问。如把每个Key的失效时间都加个随机值，<code v-pre>setRedis（Key，value，time + Math.random() * 10000）；</code>，保证数据不会在同一时间大面积失效。</p>
</li>
<li>
...
<h3 id="_20-什么是缓存预热" tabindex="-1"><a class="header-anchor" href="#_20-什么是缓存预热" aria-hidden="true">#</a> 20. 什么是缓存预热?</h3>
...
<ul>
<li>
<p>数据量不大的时候，工程启动的时候进行加载缓存动作；</p>
</li>
<li>
<p>数据量大的时候，设置一个定时任务脚本，进行缓存的刷新；</p>
</li>
<li>
<p>数据量太大的时候，优先保证热点数据进行提前加载到缓存。</p>
</li>
</ul>
<h3 id="_21-什么是缓存降级" tabindex="-1"><a class="header-anchor" href="#_21-什么是缓存降级" aria-hidden="true">#</a> 21. 什么是缓存降级？</h3>
...
<h2 id="线程模型" tabindex="-1"><a class="header-anchor" href="#线程模型" aria-hidden="true">#</a> 线程模型</h2>
<h3 id="_22-redis为何选择单线程" tabindex="-1"><a class="header-anchor" href="#_22-redis为何选择单线程" aria-hidden="true">#</a> 22. Redis为何选择单线程？</h3>
<p>在Redis 6.0以前，Redis的核心网络模型选择用单线程来实现。先来看下官方的回答：</p>
<blockquote>
...
<h3 id="_23-redis真的是单线程" tabindex="-1"><a class="header-anchor" href="#_23-redis真的是单线程" aria-hidden="true">#</a> 23. Redis真的是单线程？</h3>
...
<p>在 Redisv4.0 之后增加了一些的非阻塞命令如 <code v-pre>UNLINK</code>、<code v-pre>FLUSHALL ASYNC</code>、<code v-pre>FLUSHDB ASYNC</code>。</p>
</blockquote>
<h3 id="_24-redis-6-0为何引入多线程" tabindex="-1"><a class="header-anchor" href="#_24-redis-6-0为何引入多线程" aria-hidden="true">#</a> 24. Redis 6.0为何引入多线程？</h3>
<p>很简单，就是 Redis的网络 I/O 瓶颈已经越来越明显了。</p>
...
<h3 id="_25-redis-6-0-采用多线程后-性能的提升效果如何" tabindex="-1"><a class="header-anchor" href="#_25-redis-6-0-采用多线程后-性能的提升效果如何" aria-hidden="true">#</a> 25. Redis 6.0 采用多线程后，性能的提升效果如何？</h3>
...
<h3 id="_26-介绍下redis的线程模型" tabindex="-1"><a class="header-anchor" href="#_26-介绍下redis的线程模型" aria-hidden="true">#</a> 26. 介绍下Redis的线程模型</h3>
<p>Redis的线程模型包括Redis 6.0之前和Redis 6.0。</p>
...
<h3 id="_27-redis-6-0-多线程的实现机制" tabindex="-1"><a class="header-anchor" href="#_27-redis-6-0-多线程的实现机制" aria-hidden="true">#</a> 27. Redis 6.0 多线程的实现机制？</h3>
...
<h3 id="_28-redis-6-0开启多线程后-是否会存在线程并发安全问题" tabindex="-1"><a class="header-anchor" href="#_28-redis-6-0开启多线程后-是否会存在线程并发安全问题" aria-hidden="true">#</a> 28. Redis 6.0开启多线程后，是否会存在线程并发安全问题？</h3>
<p>从实现机制可以看出，Redis 的多线程部分只是用来处理网络数据的读写和协议解析，执行命令仍然是单线程顺序执行。</p>
<p>所以我们不需要去考虑控制 Key、Lua、事务，LPUSH/LPOP 等等的并发及线程安全问题。</p>
<h3 id="_29-redis-6-0-与-memcached-多线程模型的对比" tabindex="-1"><a class="header-anchor" href="#_29-redis-6-0-与-memcached-多线程模型的对比" aria-hidden="true">#</a> 29. Redis 6.0 与 Memcached 多线程模型的对比</h3>
<ul>
<li>
...
<h3 id="_30-redis事务的概念" tabindex="-1"><a class="header-anchor" href="#_30-redis事务的概念" aria-hidden="true">#</a> 30. Redis事务的概念</h3>
<p>Redis的事务并不是我们传统意义上理解的事务，我们都知道 单个 Redis 命令的执行是原子性的，但 Redis 没有在事务上增加任何维持原子性的机制，所以 Redis <strong>事务的执行并不是原子性的</strong>。</p>
...
<h3 id="_31-redis事务的三个阶段" tabindex="-1"><a class="header-anchor" href="#_31-redis事务的三个阶段" aria-hidden="true">#</a> 31. Redis事务的三个阶段</h3>
<ol>
<li>multi 开启事务</li>
<li>大量指令入队</li>
...
<h3 id="_32-redis事务相关命令" tabindex="-1"><a class="header-anchor" href="#_32-redis事务相关命令" aria-hidden="true">#</a> 32. Redis事务相关命令</h3>
...
<li>EXEC：执行所有事务块内的命令。返回事务块内所有命令的返回值，按命令执行的先后顺序排列。 当操作被打断时，返回空值 nil 。
通过调用DISCARD，客户端可以清空事务队列，并放弃执行事务， 并且客户端会从事务状态中退出。</li>
<li>UNWATCH命令可以取消watch对所有key的监控。</li>
</ul>
<h3 id="_33-redis事务支持隔离性吗" tabindex="-1"><a class="header-anchor" href="#_33-redis事务支持隔离性吗" aria-hidden="true">#</a> 33. Redis事务支持隔离性吗?</h3>
<p>Redis 是单进程程序，并且它保证在执行事务时，不会对事务进行中断，事务可以运行直到执行完所有事务队列中的命令为止。因此，<strong>Redis 的事务是总是带有隔离性的</strong>。</p>
<h3 id="_34-redis为什么不支持事务回滚" tabindex="-1"><a class="header-anchor" href="#_34-redis为什么不支持事务回滚" aria-hidden="true">#</a> 34. Redis为什么不支持事务回滚？</h3>
<ul>
...
<h3 id="_35-redis事务其他实现" tabindex="-1"><a class="header-anchor" href="#_35-redis事务其他实现" aria-hidden="true">#</a> 35. Redis事务其他实现</h3>
<ul>
<li>基于Lua脚本，Redis可以保证脚本内的命令一次性、按顺序地执行，
其同时也不提供事务运行错误的回滚，执行过程中如果部分命令运行错误，剩下的命令还是会继续运行完。</li>
<li>基于中间标记变量，通过另外的标记变量来标识事务是否执行完成，读取数据时先读取该标记变量判断是否事务执行完成。但这样会需要额外写代码实现，比较繁琐。</li>
</ul>
<h2 id="主从、哨兵、集群" tabindex="-1"><a class="header-anchor" href="#主从、哨兵、集群" aria-hidden="true">#</a> 主从、哨兵、集群</h2>
<h3 id="_36-redis常见使用方式有哪些" tabindex="-1"><a class="header-anchor" href="#_36-redis常见使用方式有哪些" aria-hidden="true">#</a> 36. Redis常见使用方式有哪些？</h3>
...
<h3 id="_37-介绍下redis单副本" tabindex="-1"><a class="header-anchor" href="#_37-介绍下redis单副本" aria-hidden="true">#</a> 37. 介绍下Redis单副本</h3>
...
<h3 id="_38-介绍下redis多副本-主从" tabindex="-1"><a class="header-anchor" href="#_38-介绍下redis多副本-主从" aria-hidden="true">#</a> 38. 介绍下Redis多副本（主从）</h3>
<p>Redis多副本，采用主从（replication）部署结构，相较于单副本而言最大的特点就是主从实例间数据实时同步，并且提供数据持久化和备份策略。主从实例部署在不同的物理服务器上，根据公司的基础环境配置，可以实现同时对外提供服务和读写分离策略。</p>
...
<ul>
<li>
<p>故障恢复复杂，如果没有RedisHA系统（需要开发），当主库节点出现故障时，需要手动将一个从节点晋升为主节点，同时需要通知业务方变更配置，并且需要让其它从库节点去复制新主库节点，整个过程需要人为干预，比较繁琐；</p>
</li>
<li>
<p>主库的写能力受到单机的限制，可以考虑分片；</p>
</li>
<li>
<p>主库的存储能力受到单机的限制，可以考虑Pika；</p>
</li>
<li>
<p>原生复制的弊端在早期的版本中也会比较突出，如：Redis复制中断后，Slave会发起psync，此时如果同步不成功，则会进行全量同步，主库执行全量备份的同时可能会造成毫秒或秒级的卡顿；又由于COW机制，导致极端情况下的主库内存溢出，程序异常退出或宕机；主库节点生成备份文件导致服务器磁盘IO和CPU（压缩）资源消耗；发送数GB大小的备份文件导致服务器出口带宽暴增，阻塞请求，建议升级到最新版本。</p>
</li>
</ul>
<h3 id="_39-介绍下redis-sentinel-哨兵" tabindex="-1"><a class="header-anchor" href="#_39-介绍下redis-sentinel-哨兵" aria-hidden="true">#</a> 39. 介绍下Redis Sentinel（哨兵）</h3>
<blockquote>
...
<h3 id="_40-介绍下redis-cluster" tabindex="-1"><a class="header-anchor" href="#_40-介绍下redis-cluster" aria-hidden="true">#</a> 40. 介绍下Redis Cluster</h3>
<blockquote>
...
<h3 id="_41-介绍下redis自研" tabindex="-1"><a class="header-anchor" href="#_41-介绍下redis自研" aria-hidden="true">#</a> 41. 介绍下Redis自研</h3>
<p>Redis自研的高可用解决方案，主要体现在配置中心、故障探测和failover的处理机制上，通常需要根据企业业务的实际线上环境来定制化。</p>
...
<h3 id="_42-redis高可用方案具体怎么实施" tabindex="-1"><a class="header-anchor" href="#_42-redis高可用方案具体怎么实施" aria-hidden="true">#</a> 42. Redis高可用方案具体怎么实施？</h3>
<p>使用官方推荐的哨兵(sentinel)机制就能实现，当主节点出现故障时，由Sentinel自动完成故障发现和转移，并通知应用方，实现高可用性。它有四个主要功能：</p>
<ul>
...
<h3 id="_43-了解主从复制的原理吗" tabindex="-1"><a class="header-anchor" href="#_43-了解主从复制的原理吗" aria-hidden="true">#</a> 43. 了解主从复制的原理吗？</h3>
...
<h3 id="_44-由于主从延迟导致读取到过期数据怎么处理" tabindex="-1"><a class="header-anchor" href="#_44-由于主从延迟导致读取到过期数据怎么处理" aria-hidden="true">#</a> 44. 由于主从延迟导致读取到过期数据怎么处理？</h3>
<ol>
...
<h3 id="_45-主从复制的过程中如果因为网络原因停止复制了会怎么样" tabindex="-1"><a class="header-anchor" href="#_45-主从复制的过程中如果因为网络原因停止复制了会怎么样" aria-hidden="true">#</a> 45. 主从复制的过程中如果因为网络原因停止复制了会怎么样？</h3>
...
<p>master node会在内存中创建一个<code v-pre>backlog</code>，master和slave都会保存一个<code v-pre>replica offset</code>，还有一个<code v-pre>master id</code>，offset就是保存在backlog中的。如果master和slave网络连接断掉了，slave会让master从上次的replica offset开始继续复制。</p>
<p>但是如果没有找到对应的offset，那么就会执行一次<code v-pre>resynchronization</code>全量复制。</p>
<h3 id="_46-redis主从架构数据会丢失吗-为什么" tabindex="-1"><a class="header-anchor" href="#_46-redis主从架构数据会丢失吗-为什么" aria-hidden="true">#</a> 46. Redis主从架构数据会丢失吗，为什么？</h3>
<p>有两种数据丢失的情况：</p>
<ol>
...
<h3 id="_47-如何解决主从架构数据丢失的问题" tabindex="-1"><a class="header-anchor" href="#_47-如何解决主从架构数据丢失的问题" aria-hidden="true">#</a> 47. 如何解决主从架构数据丢失的问题？</h3>
<p>数据丢失的问题是不可避免的，但是我们可以尽量减少。</p>
<p>在Redis的配置文件里设置参数</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>min-slaves-to-write 1
min-slaves-max-lag 10
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>min-slaves-to-write</code>默认情况下是0，<code v-pre>min-slaves-max-lag</code>默认情况下是10。</p>
<p>上面的配置的意思是要求至少有1个slave，数据复制和同步的延迟不能超过10秒。如果说一旦所有的slave，数据复制和同步的延迟都超过了10秒钟，那么这个时候，master就不会再接收任何请求了。</p>
<p>减小<code v-pre>min-slaves-max-lag</code>参数的值，这样就可以避免在发生故障时大量的数据丢失，一旦发现延迟超过了该值就不会往master中写入数据。</p>
<p>那么对于client，我们可以采取降级措施，将数据暂时写入本地缓存和磁盘中，在一段时间后重新写入master来保证数据不丢失；也可以将数据写入kafka消息队列，隔一段时间去消费kafka中的数据。</p>
<h3 id="_48-redis哨兵是怎么工作的" tabindex="-1"><a class="header-anchor" href="#_48-redis哨兵是怎么工作的" aria-hidden="true">#</a> 48. Redis哨兵是怎么工作的？</h3>
<ol>
<li>
<p>每个Sentinel以每秒钟一次的频率向它所知的Master，Slave以及其他 Sentinel 实例发送一个 PING 命令。</p>
</li>
<li>
<p>如果一个实例（instance）距离最后一次有效回复 PING 命令的时间超过 down-after-milliseconds 选项所指定的值， 则这个实例会被当前 Sentinel 标记为主观下线。</p>
</li>
<li>
...
<li>
<p>当Master被 Sentinel 标记为客观下线时，Sentinel 向下线的 Master 的所有 Slave 发送 INFO 命令的频率会从 10 秒一次改为每秒一次 （在一般情况下， 每个 Sentinel 会以每 10 秒一次的频率向它已知的所有Master，Slave发送 INFO 命令 ）。</p>
</li>
<li>
<p>若没有足够数量的 Sentinel 同意 Master 已经下线， Master 的客观下线状态就会变成主观下线。若 Master 重新向 Sentinel 的 PING 命令返回有效回复， Master 的主观下线状态就会被移除。</p>
</li>
<li>
<p>sentinel节点会与其他sentinel节点进行“沟通”，投票选举一个sentinel节点进行故障处理，在从节点中选取一个主节点，其他从节点挂载到新的主节点上自动复制新主节点的数据。</p>
</li>
</ol>
<h3 id="_49-故障转移时会从剩下的slave选举一个新的master-被选举为master的标准是什么" tabindex="-1"><a class="header-anchor" href="#_49-故障转移时会从剩下的slave选举一个新的master-被选举为master的标准是什么" aria-hidden="true">#</a> 49. 故障转移时会从剩下的slave选举一个新的master，被选举为master的标准是什么？</h3>
<p>如果一个master被认为odown了，而且majority哨兵都允许了主备切换，那么某个哨兵就会执行主备切换操作，此时首先要选举一个slave来，会考虑slave的一些信息。</p>
<ul>
<li>跟master断开连接的时长。
如果一个slave跟master断开连接已经超过了down-after-milliseconds的10倍，外加master宕机的时长，那么slave就被认为不适合选举为master.</li>
</ul>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>( down-after-milliseconds * 10) + milliseconds_since_master_is_in_SDOWN_state
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ul>
<li>
<p>slave优先级。
按照slave优先级进行排序，slave priority越低，优先级就越高</p>
</li>
<li>
<p>复制offset。
如果slave priority相同，那么看replica offset，哪个slave复制了越多的数据，offset越靠后，优先级就越高</p>
</li>
<li>
<p>run id
如果上面两个条件都相同，那么选择一个run id比较小的那个slave。</p>
</li>
</ul>
<h3 id="_50-同步配置的时候其他哨兵根据什么更新自己的配置呢" tabindex="-1"><a class="header-anchor" href="#_50-同步配置的时候其他哨兵根据什么更新自己的配置呢" aria-hidden="true">#</a> 50. 同步配置的时候其他哨兵根据什么更新自己的配置呢？</h3>
...
<h3 id="_51-为什么redis哨兵集群只有2个节点无法正常工作" tabindex="-1"><a class="header-anchor" href="#_51-为什么redis哨兵集群只有2个节点无法正常工作" aria-hidden="true">#</a> 51. 为什么Redis哨兵集群只有2个节点无法正常工作？</h3>
<p>哨兵集群必须部署2个以上节点。</p>
...
<h3 id="_52-redis-cluster中是如何实现数据分布的-这种方式有什么优点" tabindex="-1"><a class="header-anchor" href="#_52-redis-cluster中是如何实现数据分布的-这种方式有什么优点" aria-hidden="true">#</a> 52. Redis cluster中是如何实现数据分布的？这种方式有什么优点？</h3>
...
<h3 id="_53-redis-cluster节点间通信是什么机制" tabindex="-1"><a class="header-anchor" href="#_53-redis-cluster节点间通信是什么机制" aria-hidden="true">#</a> 53. Redis cluster节点间通信是什么机制？</h3>
<p>Redis cluster节点间采取gossip协议进行通信，所有节点都持有一份元数据，不同的节点如果出现了元数据的变更之后U不断地i将元数据发送给其他节点让其他节点进行数据变更。</p>
<blockquote>
<p>节点互相之间不断通信，保持整个集群所有节点的数据是完整的。
主要交换故障信息、节点的增加和移除、hash slot信息等。</p>
</blockquote>
<p>这种机制的好处在于，元数据的更新比较分散，不是集中在一个地方，更新请求会陆陆续续，打到所有节点上去更新，有一定的延时，降低了压力;</p>
<p>缺点，元数据更新有延时，可能导致集群的一些操作会有一些滞后。</p>
<h2 id="分布式问题" tabindex="-1"><a class="header-anchor" href="#分布式问题" aria-hidden="true">#</a> 分布式问题</h2>
<h3 id="_54-什么是分布式锁-为什么用分布式锁" tabindex="-1"><a class="header-anchor" href="#_54-什么是分布式锁-为什么用分布式锁" aria-hidden="true">#</a> 54. 什么是分布式锁？为什么用分布式锁？</h3>
<p>锁在程序中的作用就是同步工具，保证共享资源在同一时刻只能被一个线程访问，Java中的锁我们都很熟悉了，像synchronized 、Lock都是我们经常使用的，但是Java的锁只能保证单机的时候有效，分布式集群环境就无能为力了，这个时候我们就需要用到分布式锁。</p>
...
<h3 id="_55-常见的分布式锁有哪些解决方案" tabindex="-1"><a class="header-anchor" href="#_55-常见的分布式锁有哪些解决方案" aria-hidden="true">#</a> 55. 常见的分布式锁有哪些解决方案？</h3>
<p>实现分布式锁目前有三种流行方案，即基于关系型数据库、Redis、ZooKeeper 的方案</p>
<p>1、基于关系型数据库，如MySQL
基于关系型数据库实现分布式锁，是依赖数据库的唯一性来实现资源锁定，比如主键和唯一索引等。</p>
<p>缺点：</p>
<ul>
...
<h3 id="_56-redis实现分布式锁" tabindex="-1"><a class="header-anchor" href="#_56-redis实现分布式锁" aria-hidden="true">#</a> 56. Redis实现分布式锁</h3>
<h4 id="分布式锁的三个核心要素" tabindex="-1"><a class="header-anchor" href="#分布式锁的三个核心要素" aria-hidden="true">#</a> 分布式锁的三个核心要素</h4>
<p>1、加锁</p>
<p>使用setnx来加锁。key是锁的唯一标识，按业务来决定命名，value这里设置为test。</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>setx key test
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>当一个线程执行setnx返回1，说明key原本不存在，该线程成功得到了锁；当一个线程执行setnx返回0，说明key已经存在，该线程抢锁失败；</p>
<p>2、解锁</p>
<p>有加锁就得有解锁。当得到的锁的线程执行完任务，需要释放锁，以便其他线程可以进入。释放锁的最简单方式就是执行del指令。</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>del key
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>释放锁之后，其他线程就可以继续执行setnx命令来获得锁。</p>
<p>3、锁超时</p>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>expire key 30
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h4 id="上述分布式锁存在的问题" tabindex="-1"><a class="header-anchor" href="#上述分布式锁存在的问题" aria-hidden="true">#</a> 上述分布式锁存在的问题</h4>
<p><strong>通过上述<code v-pre>setnx</code> 、<code v-pre>del</code>和<code v-pre>expire</code>实现的分布式锁还是存在着一些问题。</strong></p>
...
<p>由于<code v-pre>setnx</code>指令本身是不支持传入超时时间的，而在Redis2.6.12版本上为<code v-pre>set</code>指令增加了可选参数, 用法如下：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>SET key value [EX seconds][PX milliseconds] [NX|XX]
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ul>
...
<p>具体实现参考：https://xiaomi-info.github.io/2019/12/17/Redis-distributed-lock/</p>
<h3 id="_57-了解redlock吗" tabindex="-1"><a class="header-anchor" href="#_57-了解redlock吗" aria-hidden="true">#</a> 57. 了解RedLock吗？</h3>
<p>Redlock是一种算法，Redlock也就是 Redis Distributed Lock，可用实现多节点Redis的分布式锁。</p>
...
<h3 id="_58-redlock的原理" tabindex="-1"><a class="header-anchor" href="#_58-redlock的原理" aria-hidden="true">#</a> 58. RedLock的原理</h3>
...
<li>
<p>client尝试按照顺序使用相同的key,value获取所有Redis服务的锁，在获取锁的过程中的获取时间比锁过期时间短很多，这是为了不要过长时间等待已经关闭的Redis服务。并且试着获取下一个Redis实例。</p>
</li>
</ol>
<p>比如：TTL为5s,设置获取锁最多用1s，所以如果一秒内无法获取锁，就放弃获取这个锁，从而尝试获取下个锁</p>
<ol start="3">
<li>
<p>client通过获取所有能获取的锁后的时间减去第一步的时间，这个时间差要小于TTL时间并且至少有3个Redis实例成功获取锁，才算真正的获取锁成功</p>
</li>
<li>
<p>如果成功获取锁，则锁的真正有效时间是 TTL减去第三步的时间差 的时间；比如：TTL 是5s,获取所有锁用了2s,则真正锁有效时间为3s(其实应该再减去时钟漂移);</p>
</li>
<li>
<p>如果客户端由于某些原因获取锁失败，便会开始解锁所有Redis实例；因为可能已经获取了小于3个锁，必须释放，否则影响其他client获取锁</p>
</li>
</ol>
<p>算法示意图如下：</p>
...
<h3 id="_59-redis如何做内存优化" tabindex="-1"><a class="header-anchor" href="#_59-redis如何做内存优化" aria-hidden="true">#</a> 59. Redis如何做内存优化？</h3>
<ul>
<li>
<p><strong>控制key的数量</strong>。当使用Redis存储大量数据时，通常会存在大量键，过多的键同样会消耗大量内存。Redis本质是一个数据结构服务器，它为我们提供多种数据结构，如hash，list，set，zset 等结构。使用Redis时不要进入一个误区，大量使用get/set这样的API，把Redis当成Memcached使用。对于存储相同的数据内容利用Redis的数据结构降低外层键的数量，也可以节省大量内存。</p>
</li>
<li>
<p><strong>缩减键值对象</strong>，降低Redis内存使用最直接的方式就是缩减键（key）和值（value）的长度。</p>
<ul>
...
<li>
<p><strong>编码优化</strong>。Redis对外提供了string,list,hash,set,zet等类型，但是Redis内部针对不同类型存在编码的概念，所谓编码就是具体使用哪种底层数据结构来实现。编码不同将直接影响数据的内存占用和读写效率。可参考文章：https://cloud.tencent.com/developer/article/1162213</p>
</li>
</ul>
<h3 id="_60-如果现在有个读超高并发的系统-用redis来抗住大部分读请求-你会怎么设计" tabindex="-1"><a class="header-anchor" href="#_60-如果现在有个读超高并发的系统-用redis来抗住大部分读请求-你会怎么设计" aria-hidden="true">#</a> 60. 如果现在有个读超高并发的系统，用Redis来抗住大部分读请求，你会怎么设计？</h3>
...
<blockquote>
<p>Redis单主的瓶颈不在于读写的并发，而在于内存容量，即使是一主多从也是不能解决该问题，因为一主多从架构下，多个slave的数据和master的完全一样。假如master是10G那slave也只能存10G数据。所以数据量受单主的影响。
而这个时候又需要缓存海量数据，那就必须得有多主了，并且多个主保存的数据还不能一样。Redis官方给出的 Redis cluster 模式完美的解决了这个问题。</p>
</blockquote>
