...
<h2 id="_3-并发和并行有什么区别" tabindex="-1"><a class="header-anchor" href="#_3-并发和并行有什么区别" aria-hidden="true">#</a> 3. 并发和并行有什么区别？</h2>
<p>并发就是在一段时间内，多个任务都会被处理；但在某一时刻，只有一个任务在执行。单核处理器可以做到并发。比如有两个进程<code v-pre>A</code>和<code v-pre>B</code>，<code v-pre>A</code>运行一个时间片之后，切换到<code v-pre>B</code>，<code v-pre>B</code>运行一个时间片之后又切换到<code v-pre>A</code>。因为切换速度足够快，所以宏观上表现为在一段时间内能同时运行多个程序。</p>
<p>并行就是在同一时刻，有多个任务在执行。这个需要多核处理器才能完成，在微观上就能同时执行多条指令，不同的程序被放到不同的处理器上运行，这个是物理上的多个进程同时进行。</p>
<h2 id="_4-进程与线程的切换流程" tabindex="-1"><a class="header-anchor" href="#_4-进程与线程的切换流程" aria-hidden="true">#</a> 4. 进程与线程的切换流程？</h2>
<p>进程切换分两步：</p>
...
<h2 id="_5-为什么虚拟地址空间切换会比较耗时" tabindex="-1"><a class="header-anchor" href="#_5-为什么虚拟地址空间切换会比较耗时" aria-hidden="true">#</a> 5. 为什么虚拟地址空间切换会比较耗时？</h2>
...
<h2 id="_6-进程间通信方式有哪些" tabindex="-1"><a class="header-anchor" href="#_6-进程间通信方式有哪些" aria-hidden="true">#</a> 6. 进程间通信方式有哪些？</h2>
<ul>
<li>
<p>管道：管道这种通讯方式有两种限制，一是半双工的通信，数据只能单向流动，二是只能在具有亲缘关系的进程间使用。进程的亲缘关系通常是指父子进程关系。</p>
...
<blockquote>
<p><strong>Linux系统中常用信号</strong>：
（1）<strong>SIGHUP</strong>：用户从终端注销，所有已启动进程都将收到该进程。系统缺省状态下对该信号的处理是终止进程。</p>
<p>（2）<strong>SIGINT</strong>：程序终止信号。程序运行过程中，按<code v-pre>Ctrl+C</code>键将产生该信号。</p>
<p>（3）<strong>SIGQUIT</strong>：程序退出信号。程序运行过程中，按<code v-pre>Ctrl+\\</code>键将产生该信号。</p>
<p>（4）<strong>SIGBUS和SIGSEGV</strong>：进程访问非法地址。</p>
<p>（5）<strong>SIGFPE</strong>：运算中出现致命错误，如除零操作、数据溢出等。</p>
<p>（6）<strong>SIGKILL</strong>：用户终止进程执行信号。shell下执行<code v-pre>kill -9</code>发送该信号。</p>
<p>（7）<strong>SIGTERM</strong>：结束进程信号。shell下执行<code v-pre>kill 进程pid</code>发送该信号。</p>
...
<h2 id="_7-进程间同步的方式有哪些" tabindex="-1"><a class="header-anchor" href="#_7-进程间同步的方式有哪些" aria-hidden="true">#</a> 7. 进程间同步的方式有哪些？</h2>
...
<ul>
<li>
<p>互斥量是可以命名的，也就是说它可以跨越进程使用，所以创建互斥量需要的资源更多，所以如果只为了在进程内部是用的话使用临界区会带来速度上的优势并能够减少资源占用量。</p>
</li>
<li>
...
<ul>
<li>
<p>信号量机制必须有公共内存，不能用于分布式操作系统，这是它最大的弱点；</p>
</li>
<li>
...
<h2 id="_8-线程同步的方式有哪些" tabindex="-1"><a class="header-anchor" href="#_8-线程同步的方式有哪些" aria-hidden="true">#</a> 8. 线程同步的方式有哪些？</h2>
...
<ul>
<li>
<p>互斥量与临界区的作用非常相似，但互斥量是可以命名的，也就是说互斥量可以跨越进程使用，但创建互斥量需要的资源更多，所以如果只为了在进程内部是用的话使用临界区会带来速度上的优势并能够减少资源占用量 。因为互斥量是跨进程的互斥量一旦被创建，就可以通过名字打开它。</p>
</li>
<li>
<p>互斥量，信号量，事件都可以被跨越进程使用来进行同步数据操作。</p>
</li>
</ul>
<h2 id="_9-线程的分类" tabindex="-1"><a class="header-anchor" href="#_9-线程的分类" aria-hidden="true">#</a> 9. 线程的分类？</h2>
...
<h2 id="_10-什么是临界区-如何解决冲突" tabindex="-1"><a class="header-anchor" href="#_10-什么是临界区-如何解决冲突" aria-hidden="true">#</a> 10. 什么是临界区，如何解决冲突？</h2>
...
<h2 id="_11-什么是死锁-死锁产生的条件" tabindex="-1"><a class="header-anchor" href="#_11-什么是死锁-死锁产生的条件" aria-hidden="true">#</a> 11. 什么是死锁？死锁产生的条件？</h2>
...
<h3 id="如何处理死锁问题" tabindex="-1"><a class="header-anchor" href="#如何处理死锁问题" aria-hidden="true">#</a> <strong>如何处理死锁问题</strong></h3>
<p>常用的处理死锁的方法有：死锁预防、死锁避免、死锁检测、死锁解除、鸵鸟策略。</p>
<p>**（1）死锁的预防：**基本思想就是确保死锁发生的四个必要条件中至少有一个不成立：</p>
<blockquote>
<ul>
<li>① 破除资源互斥条件</li>
...
<h2 id="_12-进程调度策略有哪几种" tabindex="-1"><a class="header-anchor" href="#_12-进程调度策略有哪几种" aria-hidden="true">#</a> 12. 进程调度策略有哪几种？</h2>
<ul>
<li>
<p><strong>先来先服务</strong>：非抢占式的调度算法，按照请求的顺序进行调度。有利于长作业，但不利于短作业，因为短作业必须一直等待前面的长作业执行完毕才能执行，而长作业又需要执行很长时间，造成了短作业等待时间过长。另外，对<code v-pre>I/O</code>密集型进程也不利，因为这种进程每次进行<code v-pre>I/O</code>操作之后又得重新排队。</p>
</li>
<li>
...
<li>
<p><strong>时间片轮转</strong>：将所有就绪进程按 <code v-pre>FCFS</code> 的原则排成一个队列，每次调度时，把 <code v-pre>CPU</code> 时间分配给队首进程，该进程可以执行一个时间片。当时间片用完时，由计时器发出时钟中断，调度程序便停止该进程的执行，并将它送往就绪队列的末尾，同时继续把 <code v-pre>CPU</code> 时间分配给队首的进程。</p>
...
<h2 id="_13-进程有哪些状态" tabindex="-1"><a class="header-anchor" href="#_13-进程有哪些状态" aria-hidden="true">#</a> 13. 进程有哪些状态？</h2>
<p>进程一共有<code v-pre>5</code>种状态，分别是创建、就绪、运行（执行）、终止、阻塞。</p>
<p><img src="http://blog-img.coolsen.cn/img/A61F5B5322ED49038C64BDD82D341987" alt="进程五种状态转换图"></p>
<ul>
<li>运行状态就是进程正在<code v-pre>CPU</code>上运行。在单处理机环境下，每一时刻最多只有一个进程处于运行状态。</li>
<li>就绪状态就是说进程已处于准备运行的状态，即进程获得了除<code v-pre>CPU</code>之外的一切所需资源，一旦得到<code v-pre>CPU</code>即可运行。</li>
<li>阻塞状态就是进程正在等待某一事件而暂停运行，比如等待某资源为可用或等待<code v-pre>I/O</code>完成。即使<code v-pre>CPU</code>空闲，该进程也不能运行。</li>
</ul>
<p><strong>运行态→阻塞态</strong>：往往是由于等待外设，等待主存等资源分配或等待人工干预而引起的。
<strong>阻塞态→就绪态</strong>：则是等待的条件已满足，只需分配到处理器后就能运行。
<strong>运行态→就绪态</strong>：不是由于自身原因，而是由外界原因使运行状态的进程让出处理器，这时候就变成就绪态。例如时间片用完，或有更高优先级的进程来抢占处理器等。
<strong>就绪态→运行态</strong>：系统按某种策略选中就绪队列中的一个进程占用处理器，此时就变成了运行态。</p>
<h2 id="_14-什么是分页" tabindex="-1"><a class="header-anchor" href="#_14-什么是分页" aria-hidden="true">#</a> 14. 什么是分页？</h2>
...
<p><img src="http://blog-img.coolsen.cn/img/image-20210610173249387.png" alt=""></p>
<h2 id="_15-什么是分段" tabindex="-1"><a class="header-anchor" href="#_15-什么是分段" aria-hidden="true">#</a> 15. 什么是分段？</h2>
...
<p><img src="http://blog-img.coolsen.cn/img/image-20210610173410509.png" alt=""></p>
<h2 id="_16-分页和分段有什区别" tabindex="-1"><a class="header-anchor" href="#_16-分页和分段有什区别" aria-hidden="true">#</a> 16. 分页和分段有什区别？</h2>
<ul>
...
<h2 id="_17-什么是交换空间" tabindex="-1"><a class="header-anchor" href="#_17-什么是交换空间" aria-hidden="true">#</a> 17. 什么是交换空间？</h2>
...
<h2 id="_18-物理地址、逻辑地址、有效地址、线性地址、虚拟地址的区别" tabindex="-1"><a class="header-anchor" href="#_18-物理地址、逻辑地址、有效地址、线性地址、虚拟地址的区别" aria-hidden="true">#</a> 18. 物理地址、逻辑地址、有效地址、线性地址、虚拟地址的区别?</h2>
<p>物理地址就是内存中真正的地址，它就相当于是你家的门牌号，你家就肯定有这个门牌号，具有唯一性。<strong>不管哪种地址，最终都会映射为物理地址</strong>。</p>
<p>在<code v-pre>实模式</code>下，段基址 + 段内偏移经过地址加法器的处理，经过地址总线传输，最终也会转换为<code v-pre>物理地址</code>。</p>
<p>但是在<code v-pre>保护模式</code>下，段基址 + 段内偏移被称为<code v-pre>线性地址</code>，不过此时的段基址不能称为真正的地址，而是会被称作为一个<code v-pre>选择子</code>的东西，选择子就是个索引，相当于数组的下标，通过这个索引能够在 GDT 中找到相应的段描述符，段描述符记录了<strong>段的起始、段的大小</strong>等信息，这样便得到了基地址。如果此时没有开启内存分页功能，那么这个线性地址可以直接当做物理地址来使用，直接访问内存。如果开启了分页功能，那么这个线性地址又多了一个名字，这个名字就是<code v-pre>虚拟地址</code>。</p>
<p>不论在实模式还是保护模式下，段内偏移地址都叫做<code v-pre>有效地址</code>。有效抵制也是逻辑地址。</p>
<p>线性地址可以看作是<code v-pre>虚拟地址</code>，虚拟地址不是真正的物理地址，但是虚拟地址会最终被映射为物理地址。下面是虚拟地址 -&gt; 物理地址的映射。</p>
<p><img src="http://blog-img.coolsen.cn/img/image-20210807152300643.png" alt="image-20210807152300643"></p>
<h2 id="_19-页面替换算法有哪些" tabindex="-1"><a class="header-anchor" href="#_19-页面替换算法有哪些" aria-hidden="true">#</a> 19. 页面替换算法有哪些？</h2>
...
<ul>
<li><code v-pre>最优算法</code>在当前页面中置换最后要访问的页面。不幸的是，没有办法来判定哪个页面是最后一个要访问的，<code v-pre>因此实际上该算法不能使用</code>。然而，它可以作为衡量其他算法的标准。</li>
<li><code v-pre>NRU</code> 算法根据 R 位和 M 位的状态将页面分为四类。从编号最小的类别中随机选择一个页面。NRU 算法易于实现，但是性能不是很好。存在更好的算法。</li>
<li><code v-pre>FIFO</code> 会跟踪页面加载进入内存中的顺序，并把页面放入一个链表中。有可能删除存在时间最长但是还在使用的页面，因此这个算法也不是一个很好的选择。</li>
<li><code v-pre>第二次机会</code>算法是对 FIFO 的一个修改，它会在删除页面之前检查这个页面是否仍在使用。如果页面正在使用，就会进行保留。这个改进大大提高了性能。</li>
<li><code v-pre>时钟</code> 算法是第二次机会算法的另外一种实现形式，时钟算法和第二次算法的性能差不多，但是会花费更少的时间来执行算法。</li>
<li><code v-pre>LRU</code> 算法是一个非常优秀的算法，但是没有<code v-pre>特殊的硬件(TLB)</code>很难实现。如果没有硬件，就不能使用 LRU 算法。</li>
<li><code v-pre>NFU</code> 算法是一种近似于 LRU 的算法，它的性能不是非常好。</li>
<li><code v-pre>老化</code> 算法是一种更接近 LRU 算法的实现，并且可以更好的实现，因此是一个很好的选择</li>
<li>最后两种算法都使用了工作集算法。工作集算法提供了合理的性能开销，但是它的实现比较复杂。<code v-pre>WSClock</code> 是另外一种变体，它不仅能够提供良好的性能，而且可以高效地实现。</li>
</ul>
<p><strong>最好的算法是老化算法和WSClock算法</strong>。他们分别是基于 LRU 和工作集算法。他们都具有良好的性能并且能够被有效的实现。还存在其他一些好的算法，但实际上这两个可能是最重要的。</p>
<h2 id="_20-什么是缓冲区溢出-有什么危害" tabindex="-1"><a class="header-anchor" href="#_20-什么是缓冲区溢出-有什么危害" aria-hidden="true">#</a> 20. 什么是缓冲区溢出？有什么危害？</h2>
...
<h2 id="_21-什么是虚拟内存" tabindex="-1"><a class="header-anchor" href="#_21-什么是虚拟内存" aria-hidden="true">#</a> 21. 什么是虚拟内存？</h2>
...
<h2 id="_22-虚拟内存的实现方式有哪些" tabindex="-1"><a class="header-anchor" href="#_22-虚拟内存的实现方式有哪些" aria-hidden="true">#</a> 22. 虚拟内存的实现方式有哪些?</h2>
<p>虚拟内存中，允许将一个作业分多次调入内存。釆用连续分配方式时，会使相当一部分内存空间都处于暂时或<code v-pre>永久</code>的空闲状态，造成内存资源的严重浪费，而且也无法从逻辑上扩大内存容量。因此，虚拟内存的实需要建立在离散分配的内存管理方式的基础上。虚拟内存的实现有以下三种方式：</p>
<ul>
<li>请求分页存储管理。</li>
<li>请求分段存储管理。</li>
<li>请求段页式存储管理。</li>
</ul>
<h2 id="_23-讲一讲io多路复用" tabindex="-1"><a class="header-anchor" href="#_23-讲一讲io多路复用" aria-hidden="true">#</a> 23. 讲一讲IO多路复用？</h2>
...
<h2 id="_24-硬链接和软链接有什么区别" tabindex="-1"><a class="header-anchor" href="#_24-硬链接和软链接有什么区别" aria-hidden="true">#</a> 24. 硬链接和软链接有什么区别？</h2>
<ul>
<li>硬链接就是在目录下创建一个条目，记录着文件名与 <code v-pre>inode</code> 编号，这个 <code v-pre>inode</code> 就是源文件的 <code v-pre>inode</code>。删除任意一个条目，文件还是存在，只要引用数量不为 <code v-pre>0</code>。但是硬链接有限制，它不能跨越文件系统，也不能对目录进行链接。</li>
<li>符号链接文件保存着源文件所在的绝对路径，在读取时会定位到源文件上，可以理解为 <code v-pre>Windows</code> 的快捷方式。当源文件被删除了，链接文件就打不开了。因为记录的是路径，所以可以为目录建立符号链接。</li>
</ul>
<h2 id="_25-中断的处理过程" tabindex="-1"><a class="header-anchor" href="#_25-中断的处理过程" aria-hidden="true">#</a> 25. 中断的处理过程?</h2>
<ol>
...
<h2 id="_26-中断和轮询有什么区别" tabindex="-1"><a class="header-anchor" href="#_26-中断和轮询有什么区别" aria-hidden="true">#</a> 26. 中断和轮询有什么区别？</h2>
<ul>
<li>轮询：CPU对<strong>特定设备</strong>轮流询问。中断：通过<strong>特定事件</strong>提醒CPU。</li>
<li>轮询：效率低等待时间长，CPU利用率不高。中断：容易遗漏问题，CPU利用率不高。</li>
</ul>
<h2 id="_27-什么是用户态和内核态" tabindex="-1"><a class="header-anchor" href="#_27-什么是用户态和内核态" aria-hidden="true">#</a> 27. 什么是用户态和内核态？</h2>
...
<h2 id="_28-用户态和内核态是如何切换的" tabindex="-1"><a class="header-anchor" href="#_28-用户态和内核态是如何切换的" aria-hidden="true">#</a> 28. 用户态和内核态是如何切换的?</h2>
<p>所有的用户进程都是运行在用户态的，但是我们上面也说了，用户程序的访问能力有限，一些比较重要的比如从硬盘读取数据，从键盘获取数据的操作则是内核态才能做的事情，而这些数据却又对用户程序来说非常重要。所以就涉及到两种模式下的转换，即<strong>用户态 -&gt; 内核态 -&gt; 用户态</strong>，而唯一能够做这些操作的只有 <code v-pre>系统调用</code>，而能够执行系统调用的就只有 <code v-pre>操作系统</code>。</p>
<p>一般用户态 -&gt; 内核态的转换我们都称之为 trap 进内核，也被称之为 <code v-pre>陷阱指令(trap instruction)</code>。</p>
<p>他们的工作流程如下：</p>
<p><img src="http://blog-img.coolsen.cn/img/image-20210807152619210.png" alt="image-20210807152619210"></p>
<ul>
<li>首先用户程序会调用 <code v-pre>glibc</code> 库，glibc 是一个标准库，同时也是一套核心库，库中定义了很多关键 API。</li>
<li>glibc 库知道针对不同体系结构调用<code v-pre>系统调用</code>的正确方法，它会根据体系结构应用程序的二进制接口设置用户进程传递的参数，来准备系统调用。</li>
<li>然后，glibc 库调用<code v-pre>软件中断指令(SWI)</code> ，这个指令通过更新 <code v-pre>CPSR</code> 寄存器将模式改为超级用户模式，然后跳转到地址 <code v-pre>0x08</code> 处。</li>
<li>到目前为止，整个过程仍处于用户态下，在执行 SWI 指令后，允许进程执行内核代码，MMU 现在允许内核虚拟内存访问</li>
<li>从地址 0x08 开始，进程执行加载并跳转到中断处理程序，这个程序就是 ARM 中的 <code v-pre>vector_swi()</code>。</li>
<li>在 vector_swi() 处，从 SWI 指令中提取系统调用号 SCNO，然后使用 SCNO 作为系统调用表 <code v-pre>sys_call_table</code> 的索引，调转到系统调用函数。</li>
<li>执行系统调用完成后，将还原用户模式寄存器，然后再以用户模式执行。</li>
</ul>
<h2 id="_29-unix-常见的io模型" tabindex="-1"><a class="header-anchor" href="#_29-unix-常见的io模型" aria-hidden="true">#</a> 29. Unix 常见的IO模型：</h2>
...
<p>对于这几种 IO 模型的详细说明，可以参考这篇文章：https://juejin.cn/post/6942686874301857800#heading-13</p>
...
<h2 id="_30-select、poll-和-epoll-之间的区别" tabindex="-1"><a class="header-anchor" href="#_30-select、poll-和-epoll-之间的区别" aria-hidden="true">#</a> 30. select、poll 和 epoll 之间的区别?</h2>
